...
test('catalog: the application module compiles and wires its dependencies', async () => {
...
test('catalog: the service is declared foundation-only, not business ready', () => {
  assert.equal(SERVICE_NAME, 'catalog');
  assert.equal(BUSINESS_READY, false);
});

test('catalog: a missing DATABASE_URL fails closed instead of guessing', () => {
...
test('catalog: the postgres probe is wired to this service own client', async () => {
  const moduleRef = await compile();
