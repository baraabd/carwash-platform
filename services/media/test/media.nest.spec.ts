...
const DSN = 'postgresql://cw_media_app:placeholder@127.0.0.1:5432/cw_media?schema=app';
...
test('media: the application module compiles and wires its dependencies', async () => {
...
test('media: the real HTTP adapter boots with distinct live/ready semantics', async () => {
  process.env.DATABASE_URL = DSN;
...
test('media: liveness reports the process is running, and its real stage', async () => {
  const moduleRef = await compile();
...
test('media: readiness answers 503 because the business API is not implemented', async () => {
  const moduleRef = await compile();
  const controller = moduleRef.get(HealthController);
...
test('media: a healthy dependency still does not make the shell ready', async () => {
  const moduleRef = await compile();
...
test('media: a failing dependency probe is reported as DOWN without leaking the DSN', async () => {
  const moduleRef = await compile();
  const options = moduleRef.get<HealthOptions>(HEALTH_OPTIONS);
...
test('media: the service is declared foundation-only, not business ready', () => {
  assert.equal(SERVICE_NAME, 'media');
  assert.equal(BUSINESS_READY, false);
});

test('media: a missing DATABASE_URL fails closed instead of guessing', () => {
...
test('media: the postgres probe is wired to this service own client', async () => {
  const moduleRef = await compile();
