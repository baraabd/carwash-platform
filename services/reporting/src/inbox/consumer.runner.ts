...
async function main(): Promise<void> {
  ...
  const consumer = new InboxConsumer<FoundationProbeCreatedV1>({
    ...
    onBeforeAck: (event, outcome) => {
      ...
      if (args.crashBeforeAckAfter > 0 && handled >= args.crashBeforeAckAfter) {
        emit({ event: 'consumer_crash_before_ack', service: 'reporting', eventId: event.eventId });
  ...
  emit({ event: 'consumer_started', service: 'reporting', queue });

  const stop = async (signal: string): Promise<void> => {
    emit({ event: 'consumer_stopping', service: 'reporting', signal, stats: consumer.stats });
    await consumer.stop().catch(() => {});
    await connection.close();
    await client.$disconnect();
    emit({ event: 'consumer_stopped', service: 'reporting', stats: consumer.stats });
    process.exit(0);
  };
