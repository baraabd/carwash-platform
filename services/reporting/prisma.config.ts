...
/**
 * Prisma configuration for the reporting service.
 *
 * DATABASE_URL is supplied by whoever is running the command and decides the
 * identity that is used:
 *   - migration jobs run as cw_reporting_migrate (may change the schema),
 *   - application replicas run as cw_reporting_app (DML only, no DDL).
 *
