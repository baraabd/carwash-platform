...
test('communications: the application module compiles and wires its dependencies', async () => {
...
test('communications: the real HTTP adapter boots with distinct live/ready semantics', async () => {
  process.env.DATABASE_URL = DSN;
...
test('communications: liveness reports the process is running, and its real stage', async () => {
  const moduleRef = await compile();
...
test('communications: a healthy dependency still does not make the shell ready', async () => {
  const moduleRef = await compile();
...
test('communications: the service is declared foundation-only, not business ready', () => {
  assert.equal(SERVICE_NAME, 'communications');
  assert.equal(BUSINESS_READY, false);
});

test('communications: a missing DATABASE_URL fails closed instead of guessing', () => {
...
test('communications: the postgres probe is wired to this service own client', async () => {
  const moduleRef = await compile();
