...
test('workforce: readiness answers 503 because the business API is not implemented', async () => {
  const moduleRef = await compile();
  const controller = moduleRef.get(HealthController);
...
test('workforce: a failing dependency probe is reported as DOWN without leaking the DSN', async () => {
  const moduleRef = await compile();
  const options = moduleRef.get<HealthOptions>(HEALTH_OPTIONS);
