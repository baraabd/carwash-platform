...
/**
 * Prisma configuration for the workforce service.
 *
 * DATABASE_URL is supplied by whoever is running the command and decides the
 * identity that is used:
 *   - migration jobs run as cw_workforce_migrate (may change the schema),
 *   - application replicas run as cw_workforce_app (DML only, no DDL).
 *
