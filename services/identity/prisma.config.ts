...
/**
 * Prisma configuration for the identity service.
 *
 * DATABASE_URL is supplied by whoever is running the command and decides the
 * identity that is used:
 *   - migration jobs run as cw_identity_migrate (may change the schema),
 *   - application replicas run as cw_identity_app (DML only, no DDL).
 *
