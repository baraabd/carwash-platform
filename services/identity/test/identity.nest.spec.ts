...
test('identity: the application module compiles and wires its dependencies', async () => {
...
test('identity: readiness answers 503 because the business API is not implemented', async () => {
  const moduleRef = await compile();
  const controller = moduleRef.get(HealthController);
...
test('identity: a failing dependency probe is reported as DOWN without leaking the DSN', async () => {
  const moduleRef = await compile();
  const options = moduleRef.get<HealthOptions>(HEALTH_OPTIONS);
...
test('identity: the service is declared foundation-only, not business ready', () => {
  assert.equal(SERVICE_NAME, 'identity');
  assert.equal(BUSINESS_READY, false);
});

test('identity: a missing DATABASE_URL fails closed instead of guessing', () => {
...
test('identity: the postgres probe is wired to this service own client', async () => {
  const moduleRef = await compile();
