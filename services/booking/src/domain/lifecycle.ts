...
export function transitionBooking(
  ...
  if (
    ...
    booking.version < 1 ||
    booking.version === Number.MAX_SAFE_INTEGER
  )
    throw new Error('INVALID_VERSION');
  if (!Number.isSafeInteger(expectedVersion) || expectedVersion !== booking.version)
    throw new Error('VERSION_CONFLICT');
  if (!Object.hasOwn(next, booking.state) || !next[booking.state].includes(target))
