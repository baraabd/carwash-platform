...
const DSN = 'postgresql://cw_booking_app:placeholder@127.0.0.1:5432/cw_booking?schema=app';
...
test('booking: the application module compiles and wires its dependencies', async () => {
...
test('booking: the real HTTP adapter boots with distinct live/ready semantics', async () => {
  process.env.DATABASE_URL = DSN;
...
test('booking: liveness reports the process is running, and its real stage', async () => {
  const moduleRef = await compile();
...
test('booking: readiness answers 503 because the business API is not implemented', async () => {
  const moduleRef = await compile();
  const controller = moduleRef.get(HealthController);
...
test('booking: a healthy dependency still does not make the shell ready', async () => {
  const moduleRef = await compile();
...
test('booking: a failing dependency probe is reported as DOWN without leaking the DSN', async () => {
  const moduleRef = await compile();
  const options = moduleRef.get<HealthOptions>(HEALTH_OPTIONS);
...
test('booking: the service is declared foundation-only, not business ready', () => {
  assert.equal(SERVICE_NAME, 'booking');
  assert.equal(BUSINESS_READY, false);
});

test('booking: a missing DATABASE_URL fails closed instead of guessing', () => {
...
test('booking: the postgres probe is wired to this service own client', async () => {
  const moduleRef = await compile();
