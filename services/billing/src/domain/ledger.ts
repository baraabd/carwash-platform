...
export function assertBalancedJournal(entries: ReadonlyArray<LedgerEntry>): void {
  if (!Array.isArray(entries) || entries.length < 2 || entries.length > 1000)
    throw new Error('INVALID_ENTRY_COUNT');
  const totals = new Map<string, bigint>();
  for (const e of entries) {
    if (typeof e.accountId !== 'string' || e.accountId.trim().length < 1)
    ...
    if (typeof e.amountMinor !== 'string' || !/^[1-9][0-9]{0,17}$/.test(e.amountMinor))
