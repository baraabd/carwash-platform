...
const DSN = 'postgresql://cw_support_app:placeholder@127.0.0.1:5432/cw_support?schema=app';
...
test('support: the application module compiles and wires its dependencies', async () => {
...
test('support: the real HTTP adapter boots with distinct live/ready semantics', async () => {
  process.env.DATABASE_URL = DSN;
...
test('support: liveness reports the process is running, and its real stage', async () => {
  const moduleRef = await compile();
...
test('support: readiness answers 503 because the business API is not implemented', async () => {
  const moduleRef = await compile();
  const controller = moduleRef.get(HealthController);
...
test('support: a healthy dependency still does not make the shell ready', async () => {
  const moduleRef = await compile();
...
test('support: a failing dependency probe is reported as DOWN without leaking the DSN', async () => {
  const moduleRef = await compile();
  const options = moduleRef.get<HealthOptions>(HEALTH_OPTIONS);
...
test('support: the service is declared foundation-only, not business ready', () => {
  assert.equal(SERVICE_NAME, 'support');
  assert.equal(BUSINESS_READY, false);
});

test('support: a missing DATABASE_URL fails closed instead of guessing', () => {
...
test('support: the postgres probe is wired to this service own client', async () => {
  const moduleRef = await compile();
